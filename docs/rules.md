...
- 共享 UI 改动应在两个分支保持一致，个人站只覆盖内容和站点配置。
...
- Projects 优先收录超过 5 star 的公开仓库，也可以收录技术代表性明确的项目系列。
- 公开资料只写可验证事实；学生身份、兴趣和个人语气可以作为自述保留，不写成外部事实。
...
- 个人站图标使用 `src/config/site.ts` 中的 `siteConfig.icon`。
